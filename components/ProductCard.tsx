    ...
    <div className="group flex flex-col overflow-hidden border border-brand-border bg-brand-surface transition duration-300 hover:-translate-y-0.5 hover:border-brand-orange/60 hover:bg-brand-surface-raised">
      <div className="relative">
        <Link href={`/products/${product.slug}`} className="block">
          <ProductVisual
            ...
            className="aspect-square w-full border-b border-brand-border"
          />
        ...
        <a
          ...
          className="absolute right-2 top-2 flex h-8 w-8 items-center justify-center border border-brand-border bg-brand-graphite/80 text-brand-steel opacity-0 backdrop-blur transition group-hover:opacity-100 hover:border-emerald-400 hover:text-emerald-400"
        >
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        <div className="flex items-center justify-between gap-2">
          <span className="font-mono-meta text-[10px] font-semibold uppercase tracking-[0.15em] text-brand-orange">
            {brand?.name ?? "Starlite"}
          </span>
          <span className="font-mono-meta text-[10px] text-brand-steel-dim">{product.sku}</span>
        </div>
        <Link href={`/products/${product.slug}`}>
          <h3 className="line-clamp-2 font-heading text-lg font-semibold uppercase leading-tight text-brand-white transition group-hover:text-brand-orange">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-3">
          ...
          <button
            ...
            className="border border-brand-orange px-3 py-2 text-[11px] font-bold uppercase tracking-[0.1em] text-brand-orange transition hover:bg-brand-orange hover:text-brand-graphite"
          >
            Add to Quote
          </button>
        </div>
